...
Lexem parseLexems_string(Sts_MetaParser_Context* ctx);
Lexem parseLexems_number(Sts_MetaParser_Context* ctx);
...
Lexems Lexems_parseLexems(Sts_MetaParser_Context* ctx, Lexems_ParseSettings settings, Source* retExpressionSource) {
  ...
  for (char c = Iter_currChar(iter);; c = Iter_currChar(iter)) {
    ...
    if (Chars_isLetter(c)) {
      Lexems_add(&lexems, parseLexems_name(ctx, settings));
    }
    else if (Chars_isDigit(c) || c == '.') {
      Lexems_add(&lexems, parseLexems_number(ctx));
    }
    else if (c == '"') {
      Lexems_add(&lexems, parseLexems_string(ctx));
    }
      ...
      Lexems_add(&lexems, parseLexems_operator(ctx, settings));
    }
  }
